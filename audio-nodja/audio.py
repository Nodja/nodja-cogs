...
ALLOWED_FILES = (".mp3", ".flac", ".ogg", ".m4a", ".wav", ".opus")
...
class Audio(commands.Cog):
        ...
        if event_type == lavalink.LavalinkEvents.TRACK_START and notify:
            ...
            if notify_channel:
                ...
                if "localtracks/" in player.current.uri:
                    ...
                    description = "**[{} - {}]({})**".format(player.current.author, player.current.title, player.current.uri)
    ...
    async def scramble(self, ctx):
        ...
        if dj_enabled:
            if not await self._can_instaskip(ctx, ctx.author):
                return await self._embed_msg(ctx, _("You need the DJ role to scramble tracks."))
        
        
        random.shuffle(player.queue)

        await self._embed_msg(
            ctx, _(f"Scrambled {len(player.queue)} tracks.")
        )
    ...
    async def now(self, ctx):
        ...
        if player.current:
            ...
            if "localtracks" in player.current.uri:
                ...
                song = "**[{track.author} - {track.title}]({track.uri})**\n"
    ...
    async def pause(self, ctx):
        ...
        if "localtracks/" in player.current.uri:
            description = "**{} - {}**\n{}".format(
                player.current.author, player.current.title, player.current.uri.replace("localtracks/", "")
            )
        else:
            description = "**[{} - {}]({})**".format(player.current.author, player.current.title, player.current.uri)
    ...
    async def clear(self, ctx):
        ...
        nr_tracks = len(player.queue)
        player.queue = []
        await self._embed_msg(
            ctx, _(f"Cleared {nr_tracks} tracks.")
        )
